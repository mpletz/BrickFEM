...
def check_valid_row(row,invalid_list=(5,7,9,11,13)):
    # check if invalid bricks are in the design
    parts = get_bricks(row)
    #print(parts)
    for num_invalid in invalid_list:
        if num_invalid in parts:
            return 0
    return 1

def check_valid(wall):
    # Checks if a wall array is valid, i.e. does not contain forbidden bricks with the size 5,7,9,...
    # wall: array of 0 and 1
    for row in wall:
        if not check_valid_row(row,invalid_list=(5,7,9,11,13)):
            return 0
    return 1
...
def create_ass_dict(run_dir,model_name,wall_bricks,nx=6,ny=5,mesh_size=0.75):
    # make dict and wall Python file
    #
...
def create_ass_dict_mass(run_dir,model_name,wall_bricks,m=1.5,material_str='polymer',is_new=1,nx=6,ny=5):
...
make_model(assembly, e_par_wall, is_new=%i)
    """%(t_step, material_str, (nx-1)/2., ny*0.5,m*1e-6,is_new)
...
def get_fu_curve0(res_dir):
    ...
    return res_name, ho_res, rp_res[:,-1], rp_res[:,4], rp_res[:,-1], rp_res[:,0]
...
def make_energy_plot(res_dir,wall_bricks):
    ...
    ax.plot(t_rp*1000,-rfz_rp/1000,label='RFZ (kN)')

    ax.set_title(name+r' ($E_\mathrm{fric}$ = '+str(round(e_fric,4))+' Nm)')

    ax.set_xlim(0,max(t_rp*1000))
    ...
    ax.set_ylabel('energy (J) or force (kN)')
...
def make_wall_model(run_dir,con_arr=(),if_run=1,nx=6,ny=5,m=0,is_new=1,mesh_size=0.75):
    """Run the BrickFEM model for a Lego wall. 

    Args:
        run_dir (str): Directory where the folder for the design is created
        var_name (str): Folder name for the design
        con_arr (tuple, optional): Definition of the bricks in the Lego wall with width of nx(studs) and height of ny(studs). The size of con_arr is (nx-1) x ny. 1 if bricks are together at that position and 0 for separate bricks. For example, the bottom row can be (0,0,1,1,1), which corresponds to these bricks 1x1, 1x1, 1x4. If con_arr is (), a random design is created. Defaults to ().
        if_run (int, optional): If the model should be run by starting Abaqus from the command line. Defaults to 1.
        nx (int, optional): Number of studs in the width of the wall. Defaults to 6.
        ny (int, optional): Number of bricks over the height of the wall. Defaults to 5.

    Returns:
        float: Total dissipated frictional energy in the model
    """
    model_dir = run_dir+'-expl-mesh'+str(int(mesh_size*100)).zfill(3)+'mm'
    print(model_dir)
    ...
    # plot the wall
    #plot_wall(wall_bricks,'img_designs/'+var_name,0)

    # create the Abaqus/Python file
    if m == 0:
        create_ass_dict(run_dir,var_name,wall_bricks,nx,ny,mesh_size=mesh_size)
    else:
        create_ass_dict_mass(run_dir,var_name,wall_bricks,m,material_str='polymer',is_new=is_new,nx=nx,ny=ny)
    ...
    e_wk, e_kin, e_fric = make_energy_plot(model_dir,wall_bricks)
    print('...')
    return e_wk, e_kin, e_fric
...
if __name__ == "__main__":
    ...
    #make_run_wall('test-rerun','plates-full', con_arr=make_full_from_sym(np.ones((7*3,4))),if_run=1,is_new=1)
    #make_run_wall('test-rerun','alternate-1x4', con_arr=make_full_from_sym(wall_alternate1x4),
    #              if_run=1,mesh_size=0.5)
    
    # simple setup for mesh study (3x4 wall)
    make_wall_model('wall-m3', con_arr=np.array(((1,1),(1,0),(1,1),(0,1))),
                    if_run=1,mesh_size=0.75,m=3)
