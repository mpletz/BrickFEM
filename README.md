...
* `load_str`: String that is added to the model name. If the same Lego set is loaded in varying ways defined in separate `explicit_par` dictionaries, it can be convenient to identify these load cases in the model name.

The sub-dictionary `loads_rigid` defines rigid parts for loading that can be either spheres (`sphere`) or cylinders (`cyl`). They both need a location of their center and a radius, stated as `loc` and `radius` in the dictionary. For the cylinder, also the `direction` of the cylinder axis and its length `len` need to be given. Note that the reference point of the cylinder lies at half of its length for the cylinder and at the center of the sphere and the location states where this center should be located.
